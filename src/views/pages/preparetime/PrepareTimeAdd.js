...
function PrepareTimeAdd(props) {
  ...
  const saveData = () => {
    if (preparetime.preparetime_name == '') {
      toast.error('PrepareTimes Name Cannot be empty!')
    } else {
      http
        .post('/preparetimes/store', preparetime)
        .then((res) => {
    ...
    <Modal size="md" isOpen={props.modalStates} toggle={props.setModalStates} centered>
      <ModalHeader toggle={props.setModalStates}>
        <div className="d-flex justify-content-between">
          <div className="">Add PrepareTimes</div>
        </div>
      </ModalHeader>
      <ModalBody className="rounded">
        <div className="  p-2 ">
          <div>
            <div className="fw-bold mb-2">PrepareTime Name</div>
            <input 
              className="form-control"
              placeholder="PrepareTime Name"
              ref={inputRef}
              value={preparetime.preparetime_name}
              onChange={(e) => {
                setPrepareTime({
                  ...preparetime,
                  preparetime_name: e.target.value,
                })
              }}
            />
          </div>
