...
function CookingDifficultyUpdate(props) {
  ...
  const saveData = () => {
    if (cookingdifficulty.cookingdifficulty_name == '') {
      toast.error('CookingDifficultys Name Cannot be empty!')
    } else {
      http
        .put(`/cookingdifficultys/update/${cookingdifficulty._id}`, cookingdifficulty)
        .then((res) => {
    ...
    <Modal size="md" isOpen={props.modalStates} toggle={props.setModalStates} centered>
      <ModalHeader toggle={props.setModalStates}>
        <div className="d-flex justify-content-between">
          <div className="">Update CookingDifficultys</div>
        </div>
      </ModalHeader>
      <ModalBody className="rounded">
        <div className="  p-2 ">
          <div>
            <div className="fw-bold mb-2">CookingDifficulty Name</div>
            <input
              className="form-control"
              placeholder="CookingDifficulty Name"
              value={cookingdifficulty.cookingdifficulty_name}
              onChange={(e) => {
                setCookingDifficulty({
                  ...cookingdifficulty,
                  cookingdifficulty_name: e.target.value,
                })
              }}
            />
          </div>
